...
CEndingScene::CEndingScene(bool isWin)
	: isWin(isWin), winImage(new CMatrix), loseImage(new CMatrix),
	isRank(false), isEnter(false), noRankMessage(new CMatrix), currentInputIndex(0), blinkTimer(0)
{
	for (int i = 0; i < 6; ++i)
	{
		num[i] = new CMatrix;
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		initial[i]->position.x = (100 + 32 * i) + (32 * i);
		initial[i]->position.y;
	}
...
void CEndingScene::Render(LPD3DXSPRITE sprite)
{
	if (isWin)
		...
		winImage->Draw(sprite, gAssetManager->GetTexture("Lose"));
