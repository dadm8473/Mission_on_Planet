...
void CMatrix::Update(float deltaTime)
{
	...
	if (camera)
	{
		pos.x -= (camera->positoin.x - camera->center.x) / zValue;
		pos.y -= (camera->positoin.y - camera->center.y) / zValue;
	}
...
void CMatrix::SetAnimaiton(float rate, int maxScene, bool repeat)
{
	animation = true;
