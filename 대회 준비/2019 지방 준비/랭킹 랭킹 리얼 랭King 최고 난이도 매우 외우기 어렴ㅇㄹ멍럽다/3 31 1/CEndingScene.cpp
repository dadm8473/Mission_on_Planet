	...
	currentInputIndex(0), blinkTimer(0)
{
	...
	for (int i = 0; i < 3; ++i)
	{
		inital[i] = new CMatrix;
		inital[i]->position.x = (100 + 32 * 6) + (32 * i);
		inital[i]->position.y = 384;
	}
...
void CEndingScene::Update(float deltaTime)
{
	...
	if (isRank)
	{
		...
		for (int i = 0; i < 3; ++i)
		{
			inital[i]->currentScene = gScoreManager->scoreData[3].initial[i];
			inital[i]->Update(deltaTime);
		}
	}
	else
		...
		for (int i = 0; i < 3; ++i)
			if (currentInputIndex >= i)
				if (currentInputIndex != i || (currentInputIndex == i && (int)blinkTimer % 2 == 0))
					inital[i]->Draw(sprite, gAssetManager->GetTexture("Apb"));
	}
	else
