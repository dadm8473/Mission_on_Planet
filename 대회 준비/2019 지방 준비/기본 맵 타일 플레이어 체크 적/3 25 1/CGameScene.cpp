...
CGameScene::CGameScene()
	...
	enemyEntryIndex(0), enemyEntryInfo(), enemyEntryTimer(0), enemyList()
{
...
void CGameScene::EnemyUpdate(float deltaTime)
{
	if (player && enemyEntryInfo.size() > 0)
	{
		...
		while (enemyEntryTimer > enemyEntryInfo[enemyEntryIndex].time)
		{
			D3DXVECTOR2 entryPos = D3DXVECTOR2(mainCamera->position.x + 1024 + 128, player->ground_Y_Pos - 200 - rand() % 100);

			CEnemy* enemy = new CEnemy(entryPos, 0, mainCamera);
			enemyList.push_back(enemy);

			enemyEntryIndex++;

			if (enemyEntryIndex >= enemyEntryInfo.size())
			{
				enemyEntryIndex = 0;
				enemyEntryTimer = 0;
				break;
